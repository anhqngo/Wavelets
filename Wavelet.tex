...
\begin{thm}[{\cite[411]{davidson}}] \label{span}
	...
	\begin{proof}
		...
		\]
		Since $ C_c(\R) $ is a dense subspace of $ L^2(\R) $, it follows that the Haar system spans all of $ L^2(\R) $.
...
Because Haar wavelet basis is orthonormal, we can easily calculate the coefficients and sort them in decreasing magnitude. Furthermore, due to its highly localization (the support for each function is located in a narrow dyadic interval), we can discard small coefficients without losing much data.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\linewidth]{img/s.png}
	\caption{Compression with Haar Wavelet Basis}
	\label{fig:s2}
\end{figure}

Figure \ref{fig:s2} shows the most significant coefficients when we apply image compression using Haar Wavelet Basis. The implementation of this compression algorithms can be found in \cite{stollnitz}.
